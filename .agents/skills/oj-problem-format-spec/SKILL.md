...
注意：最终题目解析允许嵌入 `brute.cpp`，作为帮助读者理解题目和朴素做法的代码；单解法题的正式提交代码固定放在 `## 代码` 中并引用 `main.cpp`。多解法题可以使用“多解法结构”，每个解法章节内各自放 `### 代码`。
...
- `## 暴力` 是可选章节，用于讲朴素做法和对拍基准；若展示代码，使用 `@include-code(./brute.cpp, cpp)`。
- `## 思路` 必须作为总览，说明有哪些解法，并明确正式主解是哪一个以及它对应的 `main.<ext>` 文件。
- 至少有两个二级标题以 `## 解法` 开头，才视为多解法题。
- 多解法题可以省略全局 `## 代码`。
- 每个 `## 解法...` 章节内部应包含 `### 思路`、`### 代码`、`### 复杂度`。
...
- frontmatter 后有 `[[TOC]]`。
- 单解法正文包含 `## 形式化题目`、`## 思路`、`## 代码`、`## 复杂度`、`## 总结`。
- 多解法正文包含 `## 形式化题目`、`## 思路`、至少两个 `## 解法...`、`## 总结`；可以省略全局 `## 代码`。
- 空章节使用 HTML 注释占位。
