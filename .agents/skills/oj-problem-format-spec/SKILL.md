...
- Luogu 题目优先抓取官方 `difficulty` 字段（id 与枚举一一对应：0→`未知`、1→`入门`、2→`普及-`、3→`普及/提高-`、4→`普及+/提高`、5→`提高+/省选-`、6→`省选/NOI-`、7→`NOI/NOI+/CTSC`），禁止凭印象猜测。
...
不要在题解正文中添加一级标题。除末尾的 `## 图示解析` 外，不添加二级标题；题目标题由 frontmatter 的 `title` 提供。
...
- 除 `## 图示解析` 外，如需分段，使用四级标题，例如 `#### 样例图`、`#### DP 表格`。
