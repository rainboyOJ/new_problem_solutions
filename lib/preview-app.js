...
import { RbookArticleService } from './rbook-articles.js';
...
async function pageLocals(snapshot, rbookArticleService) {
  return {
    ...
    rbookArticles: await rbookArticleService.getArticles(snapshot.problem.showAtRbook),
...
export async function buildPreviewApp(preview, opts = {}) {
  ...
  const rbookArticleService = opts.rbookArticleService || new RbookArticleService();
  ...
  app.get('/problems/:oj/:id/', async (request, reply) => {
    ...
    return reply.view('problem.pug', await pageLocals(snapshot, rbookArticleService));
  });
