...
class ProblemManager {
  ...
  load_problems() {
    ...
    try {
      const cachedProblems = JSON.parse(fs.readFileSync(__problemJsonPath, 'utf8'));
      this.problems = Array.isArray(cachedProblems)
        ? cachedProblems.map((problem) => this.normalizeProblem(problem))
        : cachedProblems;
  ...
  init() {
    ...
    for (const md of files) {
      ...
      this.problems.push(this.normalizeProblem({
        ...prob_front,
        md_path: md,
