...
function taskKeysFromMarkdown(markdown) {
  const keys = [];
  const taskReferences = String(markdown || '').matchAll(
    /^\s*-\s+\[[ xX]\][^\n]*(\[\[[^\]\n]+\]\])/gm,
  );

  for (const match of taskReferences) {
    const reference = parseProblemReference(match[1]);
    if (reference) {
      keys.push(`${reference.oj}/${reference.problem_id}`);
    }
  }
...
export class ProblemSetManager {
  constructor(problemManager, options = {}) {
    this.problemManager = problemManager;
    this.baseDir = options.baseDir || __problemSetDir;
    this.problemSets = [];
    this.problemSetMap = new Map();
  ...
  buildCatalog() {
    if (!fs.existsSync(this.baseDir)) {
      throw new Error(`Problem-set content root does not exist: ${this.baseDir}`);
    }
    ...
    for (const entry of entries) {
      try {
        ...
        const taskKeys = taskKeysFromMarkdown(parsed.content);
        ...
        errors.push({
          type: 'problem-set',
          key: slugFromFilename(entry.name),
    ...
    problemSets.sort(compareProblemSets);
    return { problemSets, errors };
  }

  activateCatalog(catalog) {
    this.problemSets = [...catalog.problemSets];
    this.problemSetMap = new Map(this.problemSets.map((item) => [item.slug, item]));
  }

  clearCatalog() {
    this.problemSets = [];
    this.problemSetMap.clear();
  }

  remove(slug) {
    const problemSet = this.problemSetMap.get(slug);
    if (!problemSet) return false;
    this.problemSets = this.problemSets.filter((candidate) => candidate !== problemSet);
    this.problemSetMap.delete(slug);
    return true;
  }
  ...
  init() {
    ...
    for (const error of catalog.errors) {
      console.warn(`忽略无效题目单 ${error.key}: ${error.message}`);
    }
    return catalog;
  }
  ...
  find(slug) {
    ...
    const metadata = this.problemSetMap.get(safeSlug);
    if (!metadata) {
      return null;
    }
    ...
    return {
      ...
      title: frontmatter.title || metadata.title
        || (firstHeading ? firstHeading[1].trim() : titleFromSlug(safeSlug)),
