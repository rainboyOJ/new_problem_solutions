...
import { readMarkdownWithIncludes } from './markdown-includes.js';
...
function createMarkdownIt(problemManager, options = {}) {
  const safeProfile = options.profile === 'ai-note';
  ...
  if (safeProfile && options.assetBaseUrl) {
    const assetBaseUrl = options.assetBaseUrl.replace(/\/$/, '');
    md.core.ruler.after('inline', 'ai_note_assets', (state) => {
      for (const token of state.tokens) {
        ...
        for (const child of token.children) {
          ...
          child.attrSet('src', `${assetBaseUrl}/${encoded.slice('assets/'.length)}`);
        }
      }
    });
  }
...
class MarkdownRenderer {
  constructor(md_path = '', problemManager = null, options = {}) {
    this.name = 'rbook';
    this.front_matter = {};
    this.md_content = '';
  ...
  readfile(md_path) {
    if (this.options.profile === 'ai-note') {
      return fs.readFileSync(md_path, 'utf8');
    }
    return readMarkdownWithIncludes(md_path);
  }
