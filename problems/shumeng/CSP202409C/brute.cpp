 ...
 */
// brute.cpp：小数据暴力解，逐块检查并直接修改文件行数组。
#include <bits/stdc++.h>
using namespace std;
...
bool read_number(const string &text, int &position, long long &value) {
...
bool parse_header(const string &line, Block &block) {
    if (line.size() < 12 || line.substr(0, 4) != "@@ -") return false;
    int position = 4;
    if (!read_number(line, position, block.start)) return false;
    if (position >= (int)line.size() || line[position++] != ',') return false;
    if (!read_number(line, position, block.old_count)) return false;
    if (position >= (int)line.size() || line[position++] != ' ') return false;
    if (position >= (int)line.size() || line[position++] != '+') return false;
    long long ignored_new_start;
    if (!read_number(line, position, ignored_new_start)) return false;
    if (position >= (int)line.size() || line[position++] != ',') return false;
    if (!read_number(line, position, block.new_count)) return false;
    if (position + 3 != (int)line.size() || line[position] != ' ' ||
...
bool matches(const vector<string> &file, long long start, const vector<string> &part) {
...
int main() {
    ...
    vector<Block> blocks;
    for (int i = 0; i < (int)raw_blocks.size() && !damaged; i++) {
        Block block;
        if (!parse_header(raw_blocks[i][0], block)) {
            damaged = true;
            break;
        }
        ...
        if (damaged || block.old_count != (long long)block.old_part.size() ||
    ...
    for (int i = 0; i < (int)blocks.size() && !damaged; i++) {
        ...
        for (long long delta = -radius; delta <= radius; delta++) {
            if ((delta > 0 && requested_start > LLONG_MAX - delta) ||
                (delta < 0 && requested_start < LLONG_MIN - delta)) continue;
            long long candidate = requested_start + delta;
            if (candidate < 1 || candidate > (long long)file.size() - (long long)block.old_part.size() + 1) continue;
            if (i > 0 && candidate < previous_end) continue;
            if (!matches(file, candidate, block.old_part)) continue;
        ...
        int begin = (int)best_start - 1;
        file.erase(file.begin() + begin, file.begin() + begin + (int)block.old_part.size());
        file.insert(file.begin() + begin, block.new_part.begin(), block.new_part.end());
        offset += best_delta;
