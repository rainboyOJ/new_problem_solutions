...
2. 若类型不是 Discover 或 Request 之一，则不处理；
...
![](https://oj.shumeng.tech/p/545/file/1.svg)

## 输入格式

从标准输入读入数据。

输入的第一行包含用空格分隔的四个正整数和一个字符串，分别是：$N,T_{def},T_{max},T_{min},H$，保证 $T_{min} \leq T_{def} \leq T_{max}$。
