...
本题给出的化学方程式由大小写字母、数字和符号（包括等号 `=`、加号 `+`、左圆括号 `(` 和右圆括号 `)`）组成，不会出现其他字符（包括空白字符，如空格、制表符等）。 化学方程式的格式与化学课本中的形式基本相同（化学式中表示元素原子个数的下标用正常文本，如 $\text{H}_2\text{O}$ 写成 `H2O`），用自然语言描述如下：
