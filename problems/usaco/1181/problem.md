...
每个测试用例包含多个独立的子测试用例，必须全部回答正确才能通过整个测试用例。输入的第一行包含 $T$（$1\le T\le 100$），为你需要求解的子测试用例的数量。以下是 $T$ 个子测试用例，每个子测试用例包含两行。第一行包含 $N$，第二行包含 $h_1,h_2,\ldots,h_N$。输入保证所有子测试用例的 $N$ 之和不超过 $10^5$。每个子测试用例的 $N$ 的值可能不同。
