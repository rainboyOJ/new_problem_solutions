...
Farmer John 的农场有 $N$ 个牛棚（$2 \leq N \leq 2\cdot 10^5$），编号为 $1 \dots N$。有 $N-1$ 条道路，每条道路连接两个牛棚，并且从任一牛棚均可通过一些道路到达任一其他牛棚。目前，第 $j$ 个牛棚中有 $h_j$ 个干草捆（$1\le h_j\le 10^9$）。
...
- 从牛棚 $3$ 到牛棚 $2$，移动 $1$ 个干草捆。
- 从牛棚 $4$ 到牛棚 $2$，移动 $2$ 个干草捆。
- 从牛棚 $2$ 到牛棚 $1$，移动 $1$ 个干草捆。

## 测试点性质

- 测试点 2-8 满足 $N\leq 5000$。
