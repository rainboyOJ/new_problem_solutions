...
$$
good(x) := win(x,s1) \land win(x,s2)
$$

它表示手势 $x$ 能同时赢 Elsie 的两个候选手势。代码中统计的 `cnt`，就是满足 `good(x)` 的手势数量。

Bessie 选择的是有序对 $(L,R)$。这个有序对合法，当且仅当：
...
$$
\lnot good(L) \land \lnot good(R)
$$

也就是左蹄不是好手势，右蹄也不是好手势。

如果好手势有 `cnt` 个，那么非好手势有 $N-cnt$ 个。左蹄有 $N-cnt$ 种选法，右蹄也有 $N-cnt$ 种选法。根据组合数学中的**乘法规则**，非法有序对数量是：
