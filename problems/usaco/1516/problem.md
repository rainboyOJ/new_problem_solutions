...
- 他不希望任何奶牛与另一头身高完全相同的奶牛相邻。形式化地说，对于所有 $1 \le i < K$，都有 $h_i \ne h_{i+1}$。
- 他希望照片左右对称。形式化地说，如果 $i+j=K+1$，那么 $h_i = h_j$。
...
- 测试点 2-3：$T\le 100$，$N \le 7$。
- 测试点 4-5：$T \le 10^4$，所有奶牛的身高不超过 10。
- 测试点 6-11：没有额外限制。
