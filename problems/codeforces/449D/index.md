...
4. 那么，从这 $f(mask)$ 个数中任意挑选元素组成非空子序列，它们的 AND 结果一定包含 $mask$。
   ...
   *(题目允许 $k \geqslant 1$，所以是 $2^{f(mask)}-1$；如果不要求非空则是 $2^{f(mask)}$，最后对结果统一处理。)*
...
4. **排除非法情况**：
   ...
   题目要求是“非空子序列”（$k \geqslant 1$），因此我们需要从总数中减去这 $1$ 种空集的情况。
...
定义 $dp[i][mask]$ 为：考虑前 $i$ 位，满足 $a_k$ 在高位（$>i$）与 $mask$ 严格一致，且在低位（$\leqslant i$）是 $mask$ 的超集的元素个数。
