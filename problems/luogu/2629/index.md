...
Uim 可以选择从第 $k$ 条消息开始汇报（$1 \leqslant k \leqslant n$），顺序为 $k, k+1, \dots, n, 1, \dots, k-1$。
问有多少个 $k$ 可以保证汇报过程中老板的心情始终 $\geqslant 0$。

**数据范围：** $n \leqslant 10^6$，$-10^3 \leqslant A_i \leqslant 10^3$。
...
对于一个起始位置 $k$（在 $A'$ 中对应区间 $[k, k+n-1]$），要保证过程中心情始终 $\geqslant 0$，意味着对于区间内任意位置 $j$ ($k \leqslant j \leqslant k+n-1$)，都要满足：
$$区间和(k, j) \geqslant 0$$
用前缀和表示就是：
$$S_j - S_{k-1} \geqslant 0 \implies S_j \geqslant S_{k-1}$$
这个条件必须对区间内**所有**的 $j$ 都成立。
显然，如果区间内**最小**的那个 $S_j$ 都满足条件，其他肯定也满足。
所以条件转化为：
$$\min_{k \leqslant j \leqslant k+n-1} \{S_j\} \geqslant S_{k-1}$$

### 难点三：滑动窗口最小值（单调队列）
我们需要枚举每一个可能的 $k$ ($1 \leqslant k \leqslant n$)。
...
3.  遍历 $i$ 从 $1$ 到 $2n-1$：
    ...
    * **判断**：当 $i \geqslant n$ 时，说明窗口长度已达到 $n$。此时对应的起点是 $k = i - n + 1$。
        检查队头（即窗口内最小值）：如果 $S[q.front()] \geqslant S[k-1]$，则该起点 $k$ 合法，答案 $+1$。
...
| 5 | 2 | 队列调整: 2<5(弹4), 2<3(弹3), 2<2(不弹) $\to$ `{1, 2, 5}`<br>队头1过期(1 < 5-4+1=2) $\to$ `{2, 5}` | $[2, 5] \to k=2$ | $S[2] = 2$ | $S[1]=-3$ | $2 \geqslant -3$ (✅) |
| 6 | 7 | `{2, 5, 6}` | $[3, 6] \to k=3$ | $S[2] = 2$ | $S[2]=2$ | $2 \geqslant 2$ (✅) |
...
## 5. 总结
* **断环成链**：解决循环数组问题的首选方法。
* **问题转化**：将“全程非负”转化为“区间内最小前缀和 $\geqslant$ 初始前缀和”。
