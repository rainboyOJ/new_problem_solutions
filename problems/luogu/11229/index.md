...
要让正整数尽可能小，第一优先级是**位数尽可能少**。因为任意 $L$ 位正整数都小于任意 $L+1$ 位正整数。一个数字最多消耗 $7$ 根木棍，所以最少位数是 $len = \lceil n/7 \rceil$。$n = 1$ 时没有任何数字能用一根木棍拼出，输出 `-1`。
...
$n = 18$ 时，最少位数是 $\lceil 18/7 \rceil = 3$。

- 第一位尝试 `1`，用掉 $2$ 根，剩 $16$ 根给 $2$ 位，超过 $2 \times 7 = 14$，不可行；
...
预处理时对每个 $i$（$2..\textit{MAXN}$），枚举首数字 $d$（`best_1` 从 $1$ 开始，`best_0` 从 $0$ 开始）。用掉 `stick[d]` 根后剩余 $rest$，如果 $rest == 1$ 跳过（$1$ 根无法拼数字），否则总位数为 `best_0[rest].len + 1`，选最优者。
