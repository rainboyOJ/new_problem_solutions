...
## 数学本质：$\mathbb{F}_2$ 上的关系系统

前面把边权称为“关系向量”。更严格地说，每个前缀值 $s[i]$ 都是二元域 $\mathbb{F}_2=\{0,1\}$ 中的元素，所有前缀值共同组成 $\mathbb{F}_2^K$ 中的一个向量；每条回答
...
因此，`a-b` 和 `b-c` 能推出 `a-c`，准确地说不是“向量具有传递性”，而是路径上的关系可以合成：

$$
rel(a,c)=rel(a,b)\oplus rel(b,c).
$$
...
在普通距离并查集中，如果从 $A$ 到 $B$ 的位移是 $+3$，那么从 $B$ 到 $A$ 的位移就是 $-3$，合并时必须小心方向。

但在 $\mathbb{F}_2$ 中，每个元素都是自己的加法逆元：
...
$$
rel(A,B)=s[A]\oplus s[B]=s[B]\oplus s[A]=rel(B,A).
$$
...
\begin{aligned}
&(s[v_0]\oplus s[v_1])
\oplus(s[v_1]\oplus s[v_2])
\oplus\cdots
\oplus(s[v_{t-1}]\oplus s[v_0])\\
&=0.
\end{aligned}
$$

每个节点势值都恰好出现两次，因此全部抵消。于是一个关系系统能够成立，当且仅当任意环上的边权异或和都是 $0$。

当 `unite(x,y,w)` 发现 $x,y$ 已经同根时，树中已经存在一条 $x$ 到 $y$ 的路径。新回答相当于再加入一条边并闭合成环：
...
并查集维护的是点与点之间的关系，而题目给出的是区间 $[l,r]$ 中 $1$ 的个数奇偶性。连接二者的是前缀异或：

$$
\bigoplus_{i=l}^{r}a_i=s[r]\oplus s[l-1].
$$
...
| 题面信号 | 应想到的转换 |
|---|---|
| 区间 $[L,R]$ 的和、奇偶性或 XOR | 建立前缀状态，把区间改写为 $L-1$ 与 $R$ 的关系 |
| 关系只有相同/相反、真/假、奇/偶 | 用 $0/1$ 表示关系，并在 $\mathbb{F}_2$ 中用 XOR 合成 |
