...
$$C_1(A_i - A_0) + C_2(B_i - B_0) \leqslant C_3$$
...
将原不等式进行移项变形：
$$C_1 A_i + C_2 B_i \leqslant C_3 + C_1 A_0 + C_2 B_0$$

令 $V_i = C_1 A_i + C_2 B_i$（每个梨子的综合权值），则约束条件变为：
$$V_i \leqslant \text{Threshold}$$

其中 $\text{Threshold} = C_3 + C_1 A_0 + C_2 B_0$。

**关键观察**：一旦确定了 $A_0$ 和 $B_0$，问题就转化为统计有多少个梨子满足：
- $A_i \geqslant A_0$ 且 $B_i \geqslant B_0$（保证 $A_0, B_0$ 确实是最小值）
- $V_i \leqslant \text{Threshold}$（满足约束条件）
...
3. 此时候选梨子只能从下标 $j \geqslant i$ 中选择（保证 $A_j \geqslant A_0$）
...
- **正面效果**：$\text{Threshold}$ 增大 → 更多梨子可能满足 $V_i \leqslant \text{Threshold}$
- **负面效果**：候选池缩小 → 必须满足 $B_i \geqslant B_0$ 的梨子减少

我们需要维护一个数据结构，支持：
1. **查询**：当前有多少梨子的 $V$ 值 ≤ 当前门槛
2. **删除**：移除不再满足 $B_i \geqslant B_0$ 的梨子
