...
因为 `dfn[v] > 0`，而未来的节点 `dfn[u] == 0`，所以“未来的节点”被条件 $p$ ($dfn[u] \geqslant dfn[v]$) 直接排除掉了。
...
2. 已知 $dfn[v] > 0$。
3. 对于任意节点 $u$，判断命题：若 $dfn[u] \geqslant dfn[v]$，则 $u$ 在 $v$ 的子树内。
...
3. **情况 C：$u$ 是 $v$ 的子孙（包括 $v$ 自己）**
   ...
   - 时间轴：$dfn[u] \geqslant dfn[v]$。
   - **结果**：满足 $p$，且 $u$ 确实在 $v$ 的子树内。$q$ 为真。

结论：

在当前时刻，所有满足 $dfn[u] \geqslant dfn[v]$ 的 $u$，只能是 情况 C。
...
2. **现象（P）**：我们观察到了一个现象 $p: dfn[u] \geqslant dfn[v]$。
...
3. **子树判断 `dfn[end_node] >= dfn[v]`**：这是一个非常实用的技巧。在 DFS 树中，如果一个点 $X$ 是点 $Y$ 的后代，那么必然有 $dfn[X] \geqslant dfn[Y]$（且退出时间 $out[X] \leqslant out[Y]$，但这题只需要比较进入时间即可，因为我们是在 $v$ 的回溯阶段判断的，只要 $B$ 被访问过了，且时间戳比 $v$ 大，它就一定在 $v$ 下面）。
