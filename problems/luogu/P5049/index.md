...
这是一个非常好的尝试。你原来的代码使用的是 $O(N^2)$ 的暴力删边法（枚举每一条环上的边，删掉，然后跑一次 $O(N)$ 的 DFS），这对于 $N \leqslant 5005$ 的普通版是可行的，但对于 **加强版 $N \leqslant 500,000$** 肯定会超时 (TLE)。
...
> 修复了一个巨大的逻辑错误: 
> 1. next_point 的更新逻辑: 下一个比v更大的非父亲子结点

@include-code(./1.4.cpp,cpp)
