...
请你选出一组牛，使得它们的总重量 $\geqslant W$，并且新的权值和 $\sum v_i$ 最大。

如果这个最大值 $\geqslant 0$，说明 $x$ 可行。
...
限制条件是：总重量 $\geqslant W$（注意：是至少 $W$，不是至多）。
...
- `dp[W]` 表示：总重量 **$\geqslant W$** 时的最大新权值和。
...
- **状态定义**：$dp[j]$ 表示重量为 $j$ (如果 $j=W$ 则表示 $\geqslant W$) 时的最大价值。
...
解决办法是：把所有 $\geqslant W$ 的状态都存到 dp[W] 里。
...
- **`dp[W]`**: 这是一个**“无限大的桶”**。所有重量 $\geqslant W$ 的方案，不管是 $W$, $W+1$, 还是 $W+10000$，最后都会掉进这个桶里，并在里面取 `max`。
