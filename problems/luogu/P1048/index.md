---
oj: "luogu"
problem_id: "P1048"
title: "[NOIP 2005 普及组] 采药"
description: "把每株草药看成只能选一次的物品，按时间做一维 0/1 背包，维护总时间不超过 t 时的最大总价值。"
difficulty: "普及-"
date: 2026-06-19 14:32
toc: true
tags: ["动态规划", "01背包", "背包"]
categories: []
pre: []
common: []
recommend: []
source: https://www.luogu.com.cn/problem/P1048
---

[[TOC]]

### 题意

给出总时间 `T` 和 `M` 株草药。

第 `i` 株草药有：

- 采它需要的时间
- 它本身的价值

要求在总采药时间不超过 `T` 的前提下，让采到的草药总价值最大。每株草药最多采一次。

### 思路

先看最直接的暴力：

@include-code(./brute.cpp, cpp)

`brute.cpp` 把每株草药看成一个 01 选择：`choose_herb[i] = 0/1` 表示不采或采。递归先生成完整选择，叶子节点再检查总时间是否超限，并统计总价值。

这个做法显然正确，但复杂度是 $O(2^M)$，只能做小数据验证。

**一句话本质：** 把每株草药看成重量和价值对应的物品，按时间做一维 01 背包——每个物品只能选一次，容量倒序。

**这个枚举卡在哪里？**

$M=100$ 时 $2^{100}$ 的量级不可能跑完。必须找到比枚举全部选择序列更紧凑的方法。

**能不能不记录"选了哪几株"，而只记"用了多少时间、拿到多少价值"？**

可以。设 $dp[t]$ 表示使用时间不超过 $t$ 时能取得的最大总价值。这个状态只关心两个数（已用时间、已得价值），不关心草药的编号和顺序。加入一株新草药时，只有两种可能：不采（状态不变）或采（从 $dp[t-time_i]$ 转移过来，加上 $value_i$）。

**为什么要倒序枚举 $t$？**

因为每株草药最多采一次。如果正序，当从 $dp[t-time_i]$ 更新 $dp[t]$ 时，$dp[t-time_i]$ 可能刚在本轮被这株草药更新过，等于同一株草药被采了多次——那是完全背包的做法。倒序保证 $dp[t-time_i]$ 还是上一轮（前 $i-1$ 株草药）的最优值。

这题本质上就是最基础的 0/1 背包：

- 每株草药是一个物品
- 采药时间是“重量”
- 草药价值是“价值”

设：

- `dp[t]` 表示总时间不超过 `t` 时，能取得的最大总价值

加入一株草药 `(time_i, value_i)` 时：

- 不采它：状态不变
- 采它：从 `dp[t - time_i]` 转移过来，再加上 `value_i`

所以转移是：

- `dp[t] = max(dp[t], dp[t - time_i] + value_i)`

由于每株草药只能采一次，时间维必须倒序枚举。

#### 状态表

这张表说明状态的含义：

| 状态 | 含义 |
| --- | --- |
| `dp[t]` | 总时间不超过 `t` 时的最大总价值 |

从这个定义可以看出，DP 只关心“用了多少时间”以及“能取得多少价值”，不需要记录具体选了哪些草药。
因此一维状态就足够。

最后输出 `dp[T]` 即可。

#### DP 公式

设 $dp_t$ 表示总时间不超过 $t$ 时能取得的最大总价值。处理第 $i$ 株草药时：

$$
dp_t=\max(dp_t,\ dp_{t-time_i}+value_i)
$$

其中 $t\geqslant time_i$。因为每株草药最多采一次，$t$ 必须倒序枚举。最终答案为：

$$
dp_T
$$


公式解释：`dp_t` 保存时间上限为 `t` 时的最大价值。选择当前草药会占用 `time_i` 时间，所以只能从剩余时间 `t-time_i` 的最优值转移过来。

#### 样例 DP 表格

以样例为例：$T = 70, M = 3$，三株草药为 $(71, 100), (69, 1), (1, 2)$。

| 处理草药 | $time_i$ | $value_i$ | $dp$ 变化（关键容量） |
| --- | --- | --- | --- |
| 初始 | — | — | $dp_{0 \sim 70} = 0$ |
| 草药 1 | 71 | 100 | 71 > 70，无法放入，$dp$ 不变 |
| 草药 2 | 69 | 1 | $dp_{69} = 1, dp_{70} = 1$ |
| 草药 3 | 1 | 2 | $dp_1 = 2, dp_{69} = 2, dp_{70} = \max(1, dp_{69}+2) = 3$ |

答案 $dp_{70} = 3$，对应采草药 2 和草药 3（耗时 $69+1=70$，价值 $1+2=3$）。


### 代码

@include-code(./main.cpp, cpp)

### 复杂度

- 时间复杂度：$O(MT)$
- 空间复杂度：$O(T)$

### 总结

这题是 0/1 背包最标准的入门模型：

- 物品只能选一次
- 容量受一个维度限制
- 目标是最大化总价值

以后看到“时间不超过多少、收益尽量大、每个对象最多用一次”这类条件，就可以优先往一维 0/1 背包上想。

### 一图流解析

这张图把本题的建模、关键转移、实现检查和训练方法压缩到一页，适合读完正文后复盘。

![一图流解析](./one-page-explainer.png)
