...
**形式化：** 在数列 $p$ 中，找出一个子段 $[l,r]$，满足 $r-l+1 \leqslant m$，最大化 $\sum_{i=l}^r p_i$。

**数据范围：** $n \leqslant 5 \times 10^5$，元素绝对值 $\leqslant 500$。
...
题目要求找出 $max(S[r] - S[l-1])$，其中 $1 \leqslant r - l + 1 \leqslant m$。
令 $j = l-1$，也就是在枚举右端点 $i$ (即原题的 $r$) 时，我们需要找到一个左端点 $j$，满足：
1.  $i - m \leqslant j < i$ （区间长度限制）
2.  使得 $S[i] - S[j]$ 最大。
...
3.  **遍历**：$i$ 从 $1$ 到 $n$：
    ...
    * **删头（维护时效性）**：检查队头元素 $idx$。我们当前计算答案时，右端点是 $i$，那么合法的左端点 $j$ 必须满足 $j \geqslant i-m$。如果 $q.front() < i - m$，说明这个 $j$ 离得太远了，超出了长度 $m$ 的限制，弹出队头。
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| **init** | $S[0]=0$ | `{0}` | - | - | - |
| **1** | $S[1]=1$ | `{0}` | 否 ($0 \geqslant 1-2$) | $1 - 0 = 1$ | $S[1]>S[0]$, 直接入队 `{0, 1}` |
| **2** | $S[2]=3$ | `{0, 1}` | 否 ($0 \geqslant 2-2$) | $3 - 0 = 3$ | $S[2]>S[1]$, 直接入队 `{0, 1, 2}` |
