...
- 覆盖函数：$\mathrm{cov}(x) = |\{ i \in S : l_i \leqslant x < r_i \}|$
- 约束：$\forall x \in \mathbb{R},\ \mathrm{cov}(x) \leqslant k$
...
*   **物理连接**：这就是**并联**。
*   **意义**：它们在空间上重叠了。为了同时维持这两个流，我们必须消耗 **2 个单位** 的总流量（即 $K$ 必须 $\geqslant 2$）。如果 $K=1$，流量不够分，就只能二选一。
...
1.  **$F_{total}$ (总流量)**：
    ...
    $$F_{total} \leqslant K \quad \text{......(公式 1)}$$
...
3.  **截面上的两种边**：
    ...
    *   **方式 B：跨越这个截面的区间边**。即所有起点 $l \leqslant v_i$ 且终点 $r \geqslant v_{i+1}$ 的区间边。设这些区间的集合为 $S_x$（即覆盖点 $x$ 的区间集合）。每个区间流量为 1。
...
**步骤 3：利用不等式推导**
我们有两个关键的物理限制：
1.  **总流量限制**：$F_{total} \leqslant K$ （源点瓶颈）。
2.  **流量非负性**：$f_{main} \geqslant 0$ （主干道流量不可能倒流，且最小为0）。

将它们代入方程：
$$ \text{count}(S_x) = F_{total} - f_{main} $$

由于 $F_{total} \leqslant K$ 且 $f_{main} \geqslant 0$，我们可以得出：
$$ \text{count}(S_x) \leqslant K - 0 $$
$$ \text{count}(S_x) \leqslant K $$
...
*   此时主干道流量 $f_{main} \geqslant 0$。
*   截面总流量 $= 3 + f_{main} \geqslant 3$。
*   但是源点总共只发出了 2 个单位的流。$2 \geqslant 3$ 显然矛盾。
...
- $0 \leqslant y_j \leqslant k$（容量约束）
...
\begin{aligned}
\min\ & \sum (-w_i) x_i \\
	ext{s.t.}\ & \sum x_i \leqslant k, \quad \text{对于所有点（通过链式边实现）} \\
& x_i \in \{0,1\}
\end{aligned}
$$
...
3. **"$\sum |z|$ 达到最大"** → 最大化权重和
4. **数据范围：$k \leqslant 3,\ n \leqslant 500$** → 费用流可行
