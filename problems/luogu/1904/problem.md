...
怎样描述一个城市的轮廓呢？Genoa 所有的建筑共享一个地面，你可以认为它是水平的。所有的建筑用一个三元组 $(L_i,H_i,R_i)$ 来表示，其中 $L_i$ 和 $R_i$ 分别是建筑的左坐标和右坐标，$H_i$ 是建筑的高度。同样表示一群建筑，在下方所示的图表中，左边对建筑物直接描述如下 $(1,11,5)$，$(2,6,7)$，$(3,13,9)$，$(12,7,16)$，$(14,3,25)$，$(19,18,22)$，$(23,13,29)$，$(24,4,28)$；右边用轮廓线来表示则可以为： $(1,11,3,13,9,0,12,7,16,3,19,18,22,3,23,13,29,0)$
![](https://cdn.luogu.com.cn/upload/image_hosting/oe7wpwsi.png)

## 输入格式

在输入数据中，你将得到一系列表示建筑的三元组。在输入数据中所有建筑的坐标中的数值都是小于 $10^4$ 的正整数，且至少有 $1$ 幢建筑，最多有 $5 \times 10^3$ 幢建筑。在输入输入中每幢建筑的三元组各占一行。三元组中的所有整数应由一个或多个空格分开。
