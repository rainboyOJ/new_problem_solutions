...
**题目大意：**
给定一个包含 $N$ ($N \leqslant 100$) 个整数的序列。你需要找到前 $P$ 个满足“非递减”性质的子序列。
输出的排序规则如下：
...
3.  **保证字典序和去重（关键点）：**
    ...
    *   $i > last\_index$（下标递增）
    *   $arr[i] \geqslant arr[last\_index]$（数值非递减）
...
4.  **可行性预处理 (DP)：**
    ...
    *   状态转移：$dp[i] = 1 + \max( \{dp[j] \mid j > i \text{ 且 } arr[j] \geqslant arr[i]\} \cup \{0\} )$。
