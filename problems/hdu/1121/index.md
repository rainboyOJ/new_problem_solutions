...
由于数据范围非常小 ($S \leqslant 100$)，我们可以直接使用一个二维数组或 `vector` 数组来模拟这个过程，复杂度完全可以接受。
...
- **空间复杂度**：我们需要存储差分表。最坏情况下，差分表是一个大小为 $S \times S$ 的三角形。因为 $S \leqslant 100$，空间大约是 $10000$ 个整数，非常小。
- **时间复杂度**：
  - 构建差分表：大约需要做 $S^2/2$ 次减法。
  - 回溯预测：对于每个要预测的数，我们需要遍历所有层（最多 $S$ 层），共预测 $C$ 个数。操作次数约 $S \times C$。
  - 总复杂度：$O(S^2 + S \times C)$。
  - 考虑到 $T$ 组数据，$S, C \leqslant 100$，总计算量在 $10^5$ 级别，远低于通常的 1秒 ($10^8$次运算) 限制，可以瞬间完成。
