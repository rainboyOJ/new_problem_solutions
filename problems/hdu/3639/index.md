...
- $N$ 为点数（$N \leqslant 5000$）
- $M$ 为边数（$M \leqslant 30000$）
- $T$ 为数据组数（$T \leqslant 50$）
...
- **单次 DFS**：在反向 DAG 上进行一次 DFS，最坏情况下会遍历整个反向图。反向图的点数 $\leqslant N$，边数 $\leqslant M$。复杂度 $O(N + M)$。
...
在竞赛中，处理这类 $N \leqslant 5000$ 的**DAG可达性统计**问题，更稳定的是使用 **`std::bitset`** 优化拓扑排序：
